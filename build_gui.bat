@echo off
REM Builds the GUI installer (mctde-installer-gui.exe). Runs from its own dir.
pushd "%~dp0"
...
cl /nologo /std:c++17 /EHsc /W3 /DUNICODE /D_UNICODE /I src /I third_party ^
   ...
   src\NameHash.cpp src\NamelistData.cpp src\Sha256.cpp src\Unpacker.cpp third_party\miniz.c build\gui.res ^
   /Fe:build\mctde-installer-gui.exe /Fo"build\\" ^
